...
class OpenAPIParserTest {
    public OpenAPIParser apiParser = new OpenAPIParser();
    ...
    void parseURLToCookRecipe() throws MalformedURLException {
        ...
        OpenAPI api = new OpenAPI(new URL(RECIPE_OPEN_API + "api/" + API_KEY +
                "COOKRCP01/json/" + 1061 + "/" + 1061));

        try {
            // when
            OpenRecipe openRecipe = apiParser.parseURLToCookRecipe();

            // then
            assertThat(openRecipe.getTotalCount()).isEqualTo(total_count);
            assertThat(openRecipe.getRow().get(0).getInfoPro()).isEqualTo(openRecipe.getRow().get(0).getInfoPro());
            assertThat(openRecipe.getRow().get(0).getInfoWgt()).isEqualTo(openRecipe.getRow().get(0).getInfoWgt());
            assertThat(openRecipe.getRow().get(0).getInfoNa()).isEqualTo(openRecipe.getRow().get(0).getInfoNa());
            assertThat(openRecipe.getRow().get(0).getRcpSeq()).isEqualTo(openRecipe.getRow().get(0).getRcpSeq());
            assertThat(openRecipe.getRow().get(0).getRcpWay2()).isEqualTo(openRecipe.getRow().get(0).getRcpWay2());
            assertThat(openRecipe.getRow().get(0).getRcpPartsDtls()).isEqualTo(openRecipe.getRow().get(0).getRcpPartsDtls());
        } catch (Exception e) {
            e.getStackTrace();
        }
    }
